...
Today we'll be focussing on the most simple function in the `ggplot2` package.
...
## Barcharts {.flexbox .vcenter}

We can then create a bar chart using the "bar" `geom`. We also need to specify the `stat` argument to be "identity". This tells `qplot` to treat the data as-is, and not to transform it at all (otherwise it tried to do a histogram).

```{r barplot, message = FALSE, fig.height=3.5, fig.width=8}
qplot(x = station, y = visits, data = visited, geom = "bar", stat = "identity")
```

## The limits of qplot

We're getting to the limits of what `qplot` can achieve for now. Further tweaks will require us to use the `ggplot()` function itself. This is more complex and beyond the scope of this session, but if you're interested check out the excellent [documentation](http://docs.ggplot2.org/current/).

For now, we'll move on to some other interesting visualisations.
