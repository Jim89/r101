...
Today we'll be using the R in the [RStudio](http://www.rstudio.com/) environment. It's the best way to use R, so we'll do a brief overview now.
...
I won't go in to detail today, but I wrote a blog post about doing that on my blog [here](http://www.thedatagent.com/tflapppart1/).
...
<br>
When making transformations, try to _add_ new fields to your data, don't change existing ones. This makes it easier to have a trail of what has happened to the data, and avoids losing track of what is raw data, and what is transformed data. 
...
In short, try not to lose sight of what you set out to do in the first place. 
...
Complex visuals may show your answer, but if they baffle the viewier, or take too much time to understand, then it's worth moving to something simpler, or adding pointers on how to read your chart.
